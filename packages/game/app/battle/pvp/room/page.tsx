...
export default function PVPBattlePage() {
  ...
  const [battle, setBattle] = useState<BattleState>({
    round: 1, playerWins: 0, opponentWins: 0, selectedAction: null, logs: [], winner: null, isProcessing: false, isPlayerTurn: true,
  })
  ...
  const canAction = (actionId: string): boolean => {
    if (battle.isProcessing || battle.winner || !battle.isPlayerTurn) return false
    switch (actionId) {
  ...
  // 对手 AI 选择行动
  const getOpponentAction = (): string => {
  ...
  const executeAction = (actor: Fighter, target: Fighter, actionId: string, isPlayer: boolean) => {
    ...
    const actorName = isPlayer ? '你' : opponent.name
    const targetName = isPlayer ? opponent.name : '你'
    let log = ''
  ...
  const executeRound = () => {
    ...
    const opponentResult = executeAction(newOpponent, newPlayer, opponentAction, false)
    newOpponent = opponentResult.newActor
    newPlayer = opponentResult.newTarget
      ...
      <main style={containerStyle}>
        <div style={resultCardStyle}>
          ...
          <div style={{ fontSize: '22px', fontWeight: 'bold', marginBottom: '8px' }}>
            {battle.winner === 'player' ? 'PVP 胜利！' : 'PVP 失败...'}
          </div>
    ...
    <main style={containerStyle}>
      <div style={gameContainerStyle}>
        ...
        <header style={headerStyle}>
          ...
          <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.5)' }}>房间: {roomId}</div>
        </header>
        ...
        <div style={roundInfoStyle}>
          ...
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '10px', color: '#10b981' }}>实时对战</div>
            <div style={{ fontSize: '12px', fontWeight: 'bold' }}>🟢 在线</div>
          </div>
        ...
        <div style={{ flex: 1, overflow: 'auto', paddingBottom: '70px' }}>
          {/* 对手 */}
          <FighterCard fighter={opponent} name={opponent.name} isOpponent />

          <div style={vsStyle}>⚔️ VS ⚔️</div>

          {/* 玩家 */}
          <FighterCard fighter={player} name={player.name} />

          {/* 战斗日志 */}
          <div style={logStyle}>
            <div style={{ fontSize: '9px', color: 'rgba(255,255,255,0.5)', marginBottom: '3px' }}>📜 战斗记录</div>
            {battle.logs.slice(0, 5).map((log, i) => (
              <div key={i} style={{
                fontSize: '9px',
                padding: '1px 0',
